        ...
        for start_x in range(0, self.array_width - chunk_array_size, chunk_array_size):
            for start_y in range(0, self.array_height - chunk_array_size, chunk_array_size):
                try:
                    ...
                    print(
#                         f"Noticed a chunk without any gene expression data: {start_x=} {start_y=} {chunk_array_size=}"
                        'Noticed a chunk without any gene expression data'
                    )
    ...
    def aggregate_clusters(self, clusters: List[List[str]], cluster_labels: List[str], aggregation: str):
        ...
        if aggregation == 'concat':
            ...
            for i, cluster in enumerate(clusters):
                gene_indexes.extend([self.genes.index(gene) for gene in cluster if gene in genes_set])
            
            dup.genes = [self.genes[index] for index in gene_indexes]
            dup.gene_ex = self.gene_ex[:, gene_indexes]
        else:
            for i, cluster in enumerate(clusters):
...
def load_training_data(id='A1', tile_size=(80, 140), verbose=False, select_genes: Optional[List[str]] = None,
device=device) -> SpatialOmicsSlide:
    if id not in data:
        raise ValueError(
            f"Invalid id: {id}. Valid IDs are {list(data.keys())}")

    # Check cache
    if os.path.exists(f'.cache/{id}.pt'):
        print("Loading slide from cache:", id)
        slide = torch.load(f'.cache/{id}.pt', map_location=torch.device('cpu'))
        return slide
    ...
    if verbose:
        print(f"Dataset {id}")
        print(
            f"Average tile size: ({average_tile_width}, {average_tile_height})")
        print(
            f"(x, y): ({min_array_x}, {min_array_y}) -> ({max_array_x}, {max_array_y})")
        print(f"Gene count:", len(matrix))
        print(f"Sample count:", len(coords))
        print()
